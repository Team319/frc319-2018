...
public class MotionProfileExample {
	...
	private SrxMotionProfile _profile;
	 ...
	 */
	public MotionProfileExample(SrxMotionProfile profile, TalonSRX talon) {
		_talon = talon;
		_profile = profile;
		
		System.out.println("created new motion profile with " + profile.numPoints + " points");
	...
	public void control() {
		...
		if (_loopTimeout < 0) {
			...
			if (_loopTimeout == 0) {
				 ...
				 * tripped
				 */
				System.out.println("something is wrong. Talon is not present, unplugged, breaker tripped?");
			} else {
				--_loopTimeout;
			}
		}
		...
		if (_talon.getControlMode() != ControlMode.MotionProfile) {
			...
			System.out.println("NOT in MotionProfile mode");
		} else {
			System.out.println("in MotionProfile mode");
			/*
	...
	private void startFilling() {
		...
		startFilling(_profile.points, _profile.numPoints);
	}

	private void startFilling(double[][] profile, int totalCnt) {
		
		System.out.println("Filling " + totalCnt + " points.");
		...
		_talon.configMotionProfileTrajectoryPeriod(0,0);
		...
		for (int i = 0; i < totalCnt; ++i) {
			...
			point.position = profile[i][0];//positionRot * Constants.kSensorUnitsPerRotation; //Convert Revolutions to Units
			point.velocity = profile[i][1];//velocityRPM * Constants.kSensorUnitsPerRotation / 600.0; //Convert RPM to Units/100ms
